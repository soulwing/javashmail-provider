...
specified in the call to the `sendMessage` method on the `Transport` class.  
Because only the envelope recipients manipulated, the message recipients in
the message header are unaffected.  This allows you to deliver all messages to
a fixed mailbox or (mailboxes) while still having the original recipient headers
in tact to inspect for correctness.
